    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <nav className="flex items-center gap-4 ml-auto">
          <Link href="/track-order" className="text-sm font-medium text-gray-700 hover:text-brand-700 hidden sm:block">
            Track Order
          </Link>
      ...
      <form onSubmit={handleSearch} className="sm:hidden px-4 pb-3 flex">
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search products..." className="input rounded-r-none" />
        <button type="submit" className="btn-primary rounded-l-none">Go</button>
      </form>
    </header>
  );
}
